...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url) {
    if (tab.url.includes('google.com/sorry/index')) {
      console.log('CAPTCHA page detected - holding automation');
      return; // Do nothing, let user solve it
    }

    if (tab.url.includes('google.com/search')) {
      console.log('Google search results ready:', tabId);
      
      // Notify sidepanel that page is ready
      chrome.runtime.sendMessage({
        action: 'googlePageLoaded',
        tabId: tabId,
        url: tab.url
      }).catch(err => {});
    }
...
setInterval(() => {
  chrome.tabs.query({ url: '*://www.google.com/*' }, (tabs) => {
    if (tabs.length > 0) {
      console.log(`Health check: ${tabs.length} Google tab(s) open`);
    }
  });
