...
const checkpointDownloadButton = document.getElementById('checkpointDownloadButton');
...
function setupEventListeners() {
    ...
    if (checkpointDownloadButton) checkpointDownloadButton.addEventListener('click', saveCheckpoint);
    if (stopRunButton) stopRunButton.addEventListener('click', stopScrapingManually);
...
function startScraping() {
    ...
    if (checkpointDownloadButton) checkpointDownloadButton.disabled = false;
    hideSection('config');
    hideSection('action');
...
function stopScrapingManually() {
    if (confirm('Stop scraping and keep current results?')) {
        finishScraping();
    }
}
...
function processNextQuery() {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]) {
            // CHECK FOR CAPTCHA BEFORE NAVIGATING
            chrome.tabs.get(tabs[0].id, (tab) => {
                if (tab.url.includes('google.com/sorry/index')) {
                    console.log('⚠️ CAPTCHA DETECTED! Pausing automation to let user solve it...');
                    showStatus('Please solve the CAPTCHA in the main window!', 'warning');
                    // We stay in waitingForPageLoad = true but don't trigger a new navigation
                    // The background.js listener will trigger triggerScrape once the user solves it and the real results load
...
function triggerScrape(tabId) {
    ...
    chrome.tabs.sendMessage(tabId, { 
        ...
        if (chrome.runtime.lastError) {
            console.error("Scrape message error:", chrome.runtime.lastError);
            handleQueryError(`Connection error: ${chrome.runtime.lastError.message}`);
            finishCurrentQuery();
        } else if (response && response.status === 'success') {
            console.log(`Scrape success: found ${response.results.length} results`);
            saveQueryResults(response.results);
            
            // Store the next page URL from Google
            ...
            if (response.results.length > 0 && scrapingState.resultsForCurrentQuery < scrapingState.maxResultsPerQuery) {
                console.log(`Pagination: Collected ${scrapingState.resultsForCurrentQuery}/${scrapingState.maxResultsPerQuery}. Moving to next page.`);
                
                if (scrapingState.nextPageUrl) {
                    console.log(`Navigating to next page URL: ${scrapingState.nextPageUrl}`);
                    scrapingState.waitingForPageLoad = true;
                    chrome.tabs.update(tabId, { url: scrapingState.nextPageUrl });
                } else {
                    console.log(`No next page URL found, trying button click fallback...`);
                    chrome.tabs.sendMessage(tabId, { action: 'clickNextPage' }, (clickResponse) => {
                        if (chrome.runtime.lastError || !clickResponse?.clicked) {
                            console.log('Pagination failed: No next button or click error');
                            finishCurrentQuery();
                        } else {
                ...
                console.log(`Query finished: reached limit or no more results.`);
                finishCurrentQuery();
            }
        } else {
            console.error("Scrape response error:", response?.error || "Unknown");
...
function finishScraping() {
    ...
    if (checkpointDownloadButton) checkpointDownloadButton.disabled = true;
...
// Add periodic checkpoint every 5 queries
function saveCheckpoint() {
    if (scrapingState.collectedResults.length > 0) {
        ...
        try {
            ...
            link.download = `google_results_checkpoint_${timestamp}.csv`;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'googlePageLoaded') {
        if (scrapingState.isRunning && scrapingState.waitingForPageLoad) {
            ...
            setTimeout(() => triggerScrape(message.tabId), 500); 
        }
    }
...
function showResults(csvData, totalQueries) {
    ...
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    const filename = `google_results_${timestamp}.csv`;
...
function downloadResults() {
    if (scrapingState.csvData) {
        const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
        downloadCSV(scrapingState.csvData, `google_results_${timestamp}.csv`);
    }
}
...
function showHelp() {
    alert("Google Scraper\n\nInput:\n- CSV with 'query' column (optional: 'run_id')\n\n1. Click Start.\n2. The tool will navigate Google automatically and collect organic results.\n\nNote: Checkpoints are saved every 5 queries.");
}
